<script>
function changetext(_tis,x)
{

  var retVal = prompt("Enter your name : ", x);
        //document.write("You have entered : " + retVal);
        if(retVal != null && retVal != '')
        {
          if(!isNaN(retVal))
          {
            updateqty(_tis,'quantity',retVal)
          }
  ...
  <tbody>
    <?php for($i=0;$i<count($select_all);$i++) { 
      ...
      <tr>
         ...
         <td><span stock_id="<?=$select_all[$i]['stock_id']?>" onclick="changetext(this,'<?=$select_all[$i]['quantity']?>')"><?=$select_all[$i]['quantity']?></span></td>
