...
$pagename="addOrder23";
$DayOrder="DayOrder23";
include('classes/dayOrder23.php');
...
<div class="container profile-view">
    <div class="col-12-xs col-sm-12 col-lg-12">
        ...
        <div class="pull-left col-md-9">
         ...
         ?>
                    <div class="form-group">
                        ...
                        <select name="root_name" onchange="fetchAll('tbl_customer','route_id',this,'shop_name')"
                            id="rote_name"
                            class="form-control {validate:{required:true,messages:{required:'<?=$required?>'}}}">
        ...
        ?>
                            <option value="<?=$roots[$i]['root_id'].'|'.$roots[$i]['root_name'] ?>" <?=$rootselected?>>
                                <?=$roots[$i]['root_name']?></option>
                            <?php }} ?>
                        </select>
                    </div>
                    <div class="form-group">
              ...
              if (isset($findByID['main_door']) && isset($_REQUEST['update_id']) && $_REQUEST['update_id'] != '')
   				 {?>
                            <option value="<?=$findByID['shop_id'].'|'.$findByID['shop_name'] ?>" selected="selected">
                                <?=$findByID['shop_name']?></option>
			 	...
			 	for($j=0;$j<count($getShops);$j++) {
                    ...
                    <div class="form-group" id="formsub_door">
                        ...
                        <select name="door_name" onchange="fetchAll('tbl_sub_doors','main_doorid',this,'sub_door_name')"
                            id="door_name"
                            class="form-control {validate:{required:true,messages:{required:'<?=$required?> Main Door'}}}">
        ...
        if(isset($findByID['main_door'])) {
            ...
            if($umain_door[0] == $maindoors[$i]['door_id'])
                $selected = "selected";
                            ...
                            <option value="<?=$maindoors[$i]['door_id'].'|'.$maindoors[$i]['door_name'] ?>"
                                <?=$selected?>><?=$maindoors[$i]['door_name']?></option>
                            <?php }} ?>
                        </select>
                    </div>
                    <div class="form-group">

                        <select name="sub_door_name" id="sub_door_name"
                            class="form-control {validate:{required:true,messages:{required:'<?=$required?>'}}}">
              ...
              if (isset($findByID['sub_door']) && isset($_REQUEST['update_id']) && $_REQUEST['update_id'] != '')
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-xs-12">
                            ...
                            <div class="col-xs-3">
                                <input type="text" placeholder="H" name="size_h1" value="<?=$findByID['size_h1']?>"
                                    class="form-control {validate:{required:true,messages:{required:'<?=$onlyrequired?>'}}}">
                            </div>
                            <div class="col-xs-3">
                                <select name="size_h2" id="size_h2"
                                    class="form-control {validate:{required:true,messages:{required:'<?=$onlyrequired?>'}}}">
        ...
        if(isset($findByID['size_h2'])) {
            ...
            if($findByID['size_h2'] == $h[$t])
                $selected = "selected";
                            ...
                            <div class="col-xs-3">
                                <input type="text" placeholder="W" name="size_w1" value="<?=$findByID['size_w1']?>"
                                    class="form-control {validate:{required:true,messages:{required:'<?=$onlyrequired?>'}}}">

                            </div>
                            <div class="col-xs-3">
                                <select name="size_w2" id="size_w2"
                                    class="form-control {validate:{required:true,messages:{required:'<?=$onlyrequired?>'}}}">
            ...
            if($findByID['size_w2'] == $h[$t])
                $selected = "selected";
                    ...
                    <div class="form-group">
                        <div class="row">
                            <div class="col-xs-4">
                                <label class="text-capitalize">Front Color:</label>
                                <select name="fcolor" id="color"
                                    class="form-control {validate:{required:true,messages:{required:'<?=$onlyrequired?>'}}}">
        ...
        if(isset($findByID['fcolor'])) {
        	
            if($findByID['fcolor'] == $colors[$t])
                $selected = "selected";
                else $selected = "";
        }?>
                            ...
                            <div class="col-xs-4">
                                <label class="text-capitalize">Back Color:</label>
                                <select name="bcolor" id="color"
                                    class="form-control">
            ...
            {
        for($t=0;$t<count($colors);$t++) {  
        if(isset($findByID['bcolor'])) {
        	
            if($findByID['bcolor'] == $colors[$t])
                $selected = "selected";
                else $selected = "";
        }?>
                                    <option value="<?=$colors[$t]?>" <?=$selected?>><?=$colors[$t]?></option>
                            ...
                            <div class="col-xs-4">
                                ...
                                <input type="number" placeholder="QTY" name="quantity" min="1" max="<?=$dayorderval ?>"
                                    value="<?=$findByID['quantity']?>"
                                    class="form-control {validate:{required:true,messages:{required:'<?=$onlyrequired?>'}}}">
                            </div>
                            <div class="col-xs-4">
    ...
    if($select_all!=0){?>
    	...
    	?>
            ...
            <tr>
                ...
                <td><a href="<?=$orderType?>?delete_id=<?= $select_all[$i]['order_id']?>"
                        onclick="return ConfirmDialog('Are you sure You Want to delete!');">Delete</a></td>
