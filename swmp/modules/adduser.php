        ...
        <form class="form-horizontal" method="post">
         ...
         ?>
          <div class="form-group">
            ...
            <input type="text" name="name" class="form-control {validate:{required:true,messages:{required:'<?=$required?>'}}}" value="<?=$findByID['name'] ?>"   placeholder="User Name">
          </div>
          <div class="form-group">
           ...
           <div class="form-group">
            ...
            <?php
            if($findByID['usertype'] === 'admin')
            $selected1 = "checked";
            else if( $findByID['usertype'] === 'exe')
            $selected2 = "checked";
            else $selected = "";
            ?>
            <input type="radio" name="usertype" value="admin" <?=$selected1?>>
            Admin
             <input type="radio" value="exe" name="usertype" <?=$selected2?>>
             executive
          </div>
  ...
  <table id="example" class="table table-bordered">
    ...
    <?php for($i=0;$i<count($select_all);$i++) {?>
      <tr>
        ...
        <td><a href="adduser?update_id=<?= $select_all[$i]['admin_id']?>">Edit</a></td>
